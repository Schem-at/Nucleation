...
`TickSimulation` loads a schematic into the `mc-tick` engine and advances it in
the game's phase order. Blocks, scheduled ticks, fluids, pistons, and entities
change headlessly. The resulting state remains queryable as data.

This is separate from the MCHPRS
[redstone simulator](redstone-simulation.md), which compiles a circuit for fast
logic evaluation. The tick engine retains update order and timing. A piston door
can depend on which opposed piston receives its notification first.

The boundary between simulation and rendering stays explicit:

- `TickSimulation` consumes a schematic and produces queryable state. It opens
  no window and requires no rendering feature.
- The renderer consumes a schematic and produces pixels. It has no tick model.
  A simulation snapshot supplies the schematic between them. See
  [rendering a run](#rendering-a-run).

The engine is checked against captures from headless Minecraft. When a capture
and the implementation disagree, the capture is the reference. Version-gated
entity loading, displacement laws, and measured hitboxes are documented in the
...
# button -> dust -> sticky piston -> stone block
...
<img src="https://raw.githubusercontent.com/Schem-at/Nucleation/master/docs/media/tick-sim/quickstart.gif?v=3" width="560" alt="The scene above: each set_block call lands one block, then the button is pressed, the sticky piston pushes the stone one cell east, and pulls it back when the button pops">
...
# stored: power=14, connected west; the first wire's shape updated too
```
...
To store the piston retracted, place it unpowered or omit the
tag. The world runs to quiescence after placement, capped at 255 ticks. A
repeater delay or piston stroke finishes before write-back.
...
`Placement` fires every observer in it because a real paste does the same.
...
<img src="https://raw.githubusercontent.com/Schem-at/Nucleation/master/docs/media/tick-sim/subtick-wave.gif?v=2" width="470" alt="One game tick played in dispatch order: the update cursor sweeping a 13x13 dust field after a button press, in position-hash order rather than a tidy radial bloom">
...
<img src="https://raw.githubusercontent.com/Schem-at/Nucleation/master/docs/media/tick-sim/subtick-heat.png?v=2" width="470" alt="Total dispatches per cell for the press: the field is hottest near the button and cools toward the corners">
...
<img src="https://raw.githubusercontent.com/Schem-at/Nucleation/master/docs/media/tick-sim/water.gif" width="560" alt="A water source placed in the middle of a walled stone basin spreading outward ring by ring until the basin is covered">
...
<img src="https://raw.githubusercontent.com/Schem-at/Nucleation/master/docs/media/tick-sim/hitbox.gif" width="560" alt="Three minecarts dropped at once: one lands on a villager's head, one rests in an oak boat, one falls straight through an armor stand to the floor">
...
<img src="https://raw.githubusercontent.com/Schem-at/Nucleation/master/docs/media/tick-sim/items.gif" width="560" alt="A dropper ejecting items on each button press; they arc out, fall, and slide to rest on the stone floor">
...
It operates on schematics rather than simulations. The bridge between the two
...
<img src="https://raw.githubusercontent.com/Schem-at/Nucleation/master/docs/media/tick-sim/render-a-run.gif" width="560" alt="The twelve snapshot frames as a flipbook: the piston scene stepping through its press-extend-retract cycle, two ticks per frame">
...
[`tests/scenarios/README.md`](https://github.com/Schem-at/Nucleation/blob/master/tests/scenarios/README.md); the engine's
...
- **Mobs and players**: no AI, no pathing, no player physics. Mobs exist as
  measured frozen hitboxes, matching how record machines use them. A mob carrying velocity refuses to load.
...
- [`crates/mc-tick/README.md`](https://github.com/Schem-at/Nucleation/blob/master/crates/mc-tick/README.md): the engine
  crate itself: coverage, verification, Rust usage.
- [`crates/mc-tick/docs/history/`](https://github.com/Schem-at/Nucleation/tree/master/crates/mc-tick/docs/history): the
