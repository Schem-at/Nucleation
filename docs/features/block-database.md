...
mappings, and a color cache derived from the vanilla texture pack). `build.rs` bakes them
...
`refresh-block-data` rewrites on every run). When Mojang ships a new release, it
regenerates the snapshots and opens or updates a PR on
`data-refresh/<version>`. The PR body
...
- `bedrock_block_states.json.gz`: PrismarineJS `data/bedrock/1.26.30/blockStates.json`
  ...
  field `1.21.60.33` is Bedrock's *state-format* version, which hasn't bumped since:   the palette content is current and includes the 26.x cinnabar/sulfur blocks). Every
