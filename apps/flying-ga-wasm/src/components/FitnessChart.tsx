...
export default function FitnessChart({ history, targetGenerations }: Props) {
  ...
  const { xOf, yOf, yTicks, xTicks, maxY } = useMemo(() => {
    ...
    const rawMax = Math.max(1, ...drawn.map((h) => h.best));
  ...
  const path = (key: "best" | "mean") =>
    drawn
      .map(
        (h, i) =>
          `${i === 0 ? "M" : "L"}${xOf(h.gen).toFixed(1)},${yOf(h[key]).toFixed(1)}`,
      )
      .join("");
