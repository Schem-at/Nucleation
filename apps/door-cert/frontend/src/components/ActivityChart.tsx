...
import type { TickEvents } from "../lib/types";
...
// shared crosshair + tooltip. Lever flips are annotated where a burst begins
// after a quiet gap.
const W = 1000;
const PANEL_H = 104;
const GAP = 26;
...
function leverFlips(events: TickEvents[]): number[] {
  const flips: number[] = [];
  let quiet = 99;
  for (const e of events) {
    const active = e.piston + e.redstone > 0;
    if (active && quiet >= 5) flips.push(e.tick);
...
export function ActivityChart({ events }: { events: TickEvents[] }) {
        ...
        {flips.map((t, i) => (
          <g key={t}>
            <line
              ...
              strokeDasharray="3 3"
            />
            <text className="annot-label" x={xOf(t) + barW / 2 + 5} y={M.top + 22}>
              lever {i % 2 === 0 ? "on" : "off"} · t={t}
            </text>
          </g>
        ))}
