...
const PASSING: &str =
    r#"{"name":"stone stays","checks":[{"tick":1,"expect":"blocks","blocks":{"0,0,0":"minecraft:stone"}}]}"#;
const FAILING: &str =
    r#"{"name":"stone is not glass","checks":[{"tick":1,"expect":"blocks","blocks":{"0,0,0":"minecraft:glass"}}]}"#;
...
fn write_corpus(dir: &Path) {
    ...
    std::fs::write(dir.join("passing.litematic"), to_litematic(&passing).unwrap()).unwrap();
...
fn run_cli(args: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_nucleation")).args(args).output().expect("the CLI runs")
}
...
fn the_grid_names_every_file_and_the_exit_code_says_fail() {
    ...
    assert_eq!(out.status.code(), Some(1), "a failing case must fail the run:\n{text}");
    ...
    assert!(text.contains("stone is not glass"), "the failure report must reach the user:\n{text}");
...
fn convert_round_trips_across_formats() {
    ...
    let out = run_cli(&["convert", source.to_str().unwrap(), "-o", schem.to_str().unwrap()]);
    assert_eq!(out.status.code(), Some(0), "{}", String::from_utf8_lossy(&out.stderr));
    let out = run_cli(&["convert", schem.to_str().unwrap(), "-o", back.to_str().unwrap()]);
    assert_eq!(out.status.code(), Some(0), "{}", String::from_utf8_lossy(&out.stderr));
...
fn diff_exit_codes_tell_identical_from_different() {
    ...
    other.set_block(1, 0, 0, &nucleation::BlockState::new("minecraft:glass".to_string()));
    ...
    assert_eq!(same.status.code(), Some(0), "{}", String::from_utf8_lossy(&same.stderr));

    let different = run_cli(&["diff", a.to_str().unwrap(), b.to_str().unwrap()]);
    assert_eq!(different.status.code(), Some(1), "{}", String::from_utf8_lossy(&different.stderr));
...
fn unknown_flags_are_usage_errors() {
    ...
    assert_eq!(out.status.code(), Some(2), "unknown subcommands are usage errors");
}
