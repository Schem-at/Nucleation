...
a comparator tracking a barrel as a hopper drains it, and item entities —
an exact three-block fall, a hopper vacuuming a falling stack, resting items
merging, and a mean-trajectory dropper eject under jitter tolerance — all
...
| **Dispensed buckets** | `DispenseItemBehavior$3`/`$4`, both directions on one rig. A filled bucket's `emptyContents` writes its contents into a **strictly air** front cell with flags 3, so an observer watching that cell pulses 2gt later; the slot is left holding `minecraft:bucket`. An empty bucket's `BucketPickup.pickupBlock` writes air (flags 11) and yields the filled bucket. A non-air front cell, or a front block that is not a `BucketPickup`, is not a refusal — `$3`/`$4` fall through to the default eject, and the slot simply empties. `consumeWithRemainder` reuses the slot when the stack is spent and otherwise runs `insertItem` into the first empty-or-matching slot. `powder_snow` and `water` are pinned (`bucket_dispense.json`, `bucket_pickup.json`); **lava agrees and is deliberately unpinned** because `minecraft:lava` has no behaviour here, so a build holding one refuses at load by name. The seven mob buckets, `bubble_column`, `waterlogged=true` and a flowing liquid level all **refuse by name** rather than approximate |
