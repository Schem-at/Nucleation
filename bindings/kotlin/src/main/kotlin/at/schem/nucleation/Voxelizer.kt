...
internal interface VoxelizerLib: Library {
    ...
    fun Voxelizer_shape_from_glb(data: Slice, targetSize: Float): ResultPointerInt
    fun Voxelizer_shape_from_obj(text: Slice, targetSize: Float): ResultPointerInt
    fun Voxelizer_schematic_from_glb_textured(data: Slice, targetSize: Float, palette: Pointer, name: Slice): ResultPointerInt
}
...
class Voxelizer internal constructor (
    ...
    companion object {
        ...
        */
        fun shapeFromGlb(data: UByteArray, targetSize: Float): Result<Shape> {
            val dataSliceMemory = PrimitiveArrayTools.borrow(data)
            
            val returnVal = lib.Voxelizer_shape_from_glb(dataSliceMemory.slice, targetSize);
            try {
        ...
        *`target_size`.
        */
        fun shapeFromObj(text: String, targetSize: Float): Result<Shape> {
            val textSliceMemory = PrimitiveArrayTools.borrowUtf8(text)
            
            val returnVal = lib.Voxelizer_shape_from_obj(textSliceMemory.slice, targetSize);
            try {
                val nativeOkVal = returnVal.getNativeOk();
                if (nativeOkVal != null) {
                    val selfEdges: List<Any> = listOf()
        ...
        *`target_size`.
        */
        fun schematicFromGlbTextured(data: UByteArray, targetSize: Float, palette: Palette, name: String): Result<Schematic> {
            val dataSliceMemory = PrimitiveArrayTools.borrow(data)
            val nameSliceMemory = PrimitiveArrayTools.borrowUtf8(name)
            
            val returnVal = lib.Voxelizer_schematic_from_glb_textured(dataSliceMemory.slice, targetSize, palette.handle, nameSliceMemory.slice);
            try {
                val nativeOkVal = returnVal.getNativeOk();
                if (nativeOkVal != null) {
                    val selfEdges: List<Any> = listOf()
