...
namespace capi {
    ...
    typedef struct Hdl_compile_blif_contract_result {union { nucleation::capi::NucleationError err;}; bool is_ok;} Hdl_compile_blif_contract_result;
    Hdl_compile_blif_contract_result Hdl_compile_blif_contract(nucleation::diplomat::capi::DiplomatStringView blif, nucleation::diplomat::capi::DiplomatStringView name, nucleation::diplomat::capi::DiplomatWrite* write);
...
inline nucleation::diplomat::result<std::string, nucleation::NucleationError> nucleation::Hdl::compile_blif_contract(std::string_view blif, std::string_view name) {
    std::string output;
    nucleation::diplomat::capi::DiplomatWrite write = nucleation::diplomat::WriteFromString(output);
    auto result = nucleation::capi::Hdl_compile_blif_contract({blif.data(), blif.size()},
        {name.data(), name.size()},
        &write);
    return result.is_ok ? nucleation::diplomat::result<std::string, nucleation::NucleationError>(nucleation::diplomat::Ok<std::string>(std::move(output))) : nucleation::diplomat::result<std::string, nucleation::NucleationError>(nucleation::diplomat::Err<nucleation::NucleationError>(nucleation::NucleationError::FromFFI(result.err)));
}
template<typename W>
inline nucleation::diplomat::result<std::monostate, nucleation::NucleationError> nucleation::Hdl::compile_blif_contract_write(std::string_view blif, std::string_view name, W& writeable) {
    nucleation::diplomat::capi::DiplomatWrite write = nucleation::diplomat::WriteTrait<W>::Construct(writeable);
    auto result = nucleation::capi::Hdl_compile_blif_contract({blif.data(), blif.size()},
        {name.data(), name.size()},
        &write);
