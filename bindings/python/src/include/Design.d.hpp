...
public:
   ...
   */
  inline nucleation::diplomat::result<std::string, nucleation::NucleationError> remove_gate(std::string_view bus, size_t index);
  template<typename W>
  inline nucleation::diplomat::result<std::monostate, nucleation::NucleationError> remove_gate_write(std::string_view bus, size_t index, W& writeable_output);
   ...
   * "changed":[...]}`.
   */
  inline nucleation::diplomat::result<std::string, nucleation::NucleationError> remove_port(std::string_view name, bool force);
  template<typename W>
  inline nucleation::diplomat::result<std::monostate, nucleation::NucleationError> remove_port_write(std::string_view name, bool force, W& writeable_output);
   ...
   */
  inline nucleation::diplomat::result<std::string, nucleation::NucleationError> route_bus_adapted(std::string_view name, std::string_view driver, std::string_view sinks_csv, std::string_view gates_json, std::string_view style_json, uint8_t align, int32_t shift, bool truncate);
  template<typename W>
  inline nucleation::diplomat::result<std::monostate, nucleation::NucleationError> route_bus_adapted_write(std::string_view name, std::string_view driver, std::string_view sinks_csv, std::string_view gates_json, std::string_view style_json, uint8_t align, int32_t shift, bool truncate, W& writeable_output);

  /**
   * The resolved bit mapping of a width-adapted bus (`null` when the
   * widths matched): `{"map":{...,"pairs":[[dbit,sbit],..]},"note":".."}`.
   */
  inline nucleation::diplomat::result<std::string, nucleation::NucleationError> bus_width_map(std::string_view name) const;
  template<typename W>
  inline nucleation::diplomat::result<std::monostate, nucleation::NucleationError> bus_width_map_write(std::string_view name, W& writeable_output) const;
   ...
   * yields `[]`.
   */
  inline nucleation::diplomat::result<std::string, nucleation::NucleationError> bus_blocks_json(std::string_view name) const;
  template<typename W>
  inline nucleation::diplomat::result<std::monostate, nucleation::NucleationError> bus_blocks_json_write(std::string_view name, W& writeable_output) const;

  /**
   * ONE instance's placed cells as `[[x,y,z,"block"],..]`, transform
   * applied. Same fast path as `bus_blocks_json`.
   */
  inline nucleation::diplomat::result<std::string, nucleation::NucleationError> instance_blocks_json(std::string_view name) const;
  template<typename W>
  inline nucleation::diplomat::result<std::monostate, nucleation::NucleationError> instance_blocks_json_write(std::string_view name, W& writeable_output) const;
