...
final class Geo {
    ...
    public static function extrudeFootprints(string $buildings_json, string $base_block, string $name) {
        ...
        $__view2->len = $__n2;
        $result = Lib::ffi()->Geo_extrude_footprints($__view0, $__view1, $__view2);
        if (!$result->is_ok) {
    ...
    public static function heightmapTerrain(string $heights_json,  $width, string $surface_block, string $subsurface_block,  $surface_depth, string $name) {
        ...
        if ($__n0 > 0) {
            $__buf0 = Lib::ffi()->new("uint8_t[" . $__n0 . "]", false);
            \FFI::memcpy($__buf0, $heights_json, $__n0);
            $__view0->data = Lib::ffi()->cast('const char*', \FFI::addr($__buf0[0]));
        } else {
            $__view0->data = null;
        }
        ...
        if ($__n5 > 0) {
            ...
            \FFI::memcpy($__buf5, $name, $__n5);
            $__view5->data = Lib::ffi()->cast('const char*', \FFI::addr($__buf5[0]));
        } else {
            $__view5->data = null;
        }
        $__view5->len = $__n5;
        $result = Lib::ffi()->Geo_heightmap_terrain($__view0, $width, $__view2, $__view3, $surface_depth, $__view5);
        if (!$result->is_ok) {
            throw new DiplomatError('NucleationError', $result->err, NucleationError::name($result->err));
        }
        return new Schematic($result->ok, true);
