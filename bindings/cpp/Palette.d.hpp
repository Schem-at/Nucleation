...
public:
   ...
   */
  inline static std::unique_ptr<Palette> materials();
   ...
   */
  inline std::unique_ptr<Palette> for_material(bool translucent) const;
   ...
   */
  inline diplomat::result<std::string, NucleationError> closest_block_dithered(uint8_t r, uint8_t g, uint8_t b, int32_t x, int32_t y, int32_t z) const;
  template<typename W>
  inline diplomat::result<std::monostate, NucleationError> closest_block_dithered_write(uint8_t r, uint8_t g, uint8_t b, int32_t x, int32_t y, int32_t z, W& writeable_output) const;
