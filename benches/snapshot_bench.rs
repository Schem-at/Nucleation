...
fn bench_serialize(c: &mut Criterion) {
    ...
    for &size in &[16, 32, 64] {
        ...
        group.bench_with_input(
            BenchmarkId::new("snapshot", format!("{}³_solid", size)),
            &schematic,
            |b, s| b.iter(|| to_snapshot(black_box(s)).unwrap()),
        );
        group.bench_with_input(
            BenchmarkId::new("schematic", format!("{}³_solid", size)),
            &schematic,
            |b, s| b.iter(|| to_schematic(black_box(s)).unwrap()),
        );
    }

    // 128³ solid — separate to avoid long setup in the loop
    ...
    // Sparse
    for &pct in &[0.1, 0.5] {
        let label = format!("32³_{}pct", (pct * 100.0) as u32);
        let schematic = make_sparse_schematic(32, pct);
    ...
    // Diverse palette
    let diverse = make_diverse_schematic(64);
    group.bench_with_input(
        BenchmarkId::new("snapshot", "64³_diverse"),
        &diverse,
        |b, s| b.iter(|| to_snapshot(black_box(s)).unwrap()),
    );
    group.bench_with_input(
        BenchmarkId::new("schematic", "64³_diverse"),
        &diverse,
        |b, s| b.iter(|| to_schematic(black_box(s)).unwrap()),
    );
...
fn bench_deserialize(c: &mut Criterion) {
    ...
    for &size in &[16, 32, 64, 128] {
        ...
        group.bench_with_input(
            ...
            &schem_bytes,
            |b, data| {
                b.iter(|| nucleation::formats::schematic::from_schematic(black_box(data)).unwrap())
            },
        );
    }
    ...
    for &size in &[16, 32, 64] {
        ...
        group.bench_with_input(BenchmarkId::new("schematic", &label), &schematic, |b, s| {
            b.iter(|| {
                ...
                nucleation::formats::schematic::from_schematic(&bytes).unwrap()
            })
        });
    }
