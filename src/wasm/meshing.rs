...
use crate::meshing::{
    build_global_atlas, ChunkMeshResult, MeshConfig, MeshOutput, MeshPhase, MeshProgress,
    NucleationChunkIter, RawMeshExport, ResourcePackSource,
};
...
#[wasm_bindgen]
pub struct TextureAtlasWrapper {
    inner: TextureAtlas,
}

#[wasm_bindgen]
impl TextureAtlasWrapper {
    ...
    pub fn to_bytes(&self) -> Uint8Array {
        Uint8Array::from(self.inner.pixels.as_slice())
    }
}
...
impl MeshOutputWrapper {
    ...
    #[wasm_bindgen(js_name = toNucm)]
    pub fn to_nucm(&self) -> Uint8Array {
        let data = crate::meshing::cache::serialize_meshes(&[self.inner.clone()]);
...
impl MultiMeshResultWrapper {
    ...
    pub fn get_mesh(&self, region_name: &str) -> Option<MeshOutputWrapper> {
        self.inner.get(region_name).map(|mesh| MeshOutputWrapper {
            inner: mesh.clone(),
        })
    }
...
impl ChunkMeshResultWrapper {
    ...
    #[wasm_bindgen(js_name = toNucm)]
    pub fn to_nucm(&self) -> Uint8Array {
        let meshes: Vec<MeshOutput> = self.inner.meshes.values().cloned().collect();
...
impl ChunkMeshIteratorWrapper {
    ...
    pub fn set_progress_callback(&mut self, callback: &js_sys::Function) {
        ...
        self.inner
            .set_progress_callback(Box::new(move |progress: MeshProgress| {
                let obj = Object::new();
                let phase_str = match progress.phase {
                    MeshPhase::BuildingAtlas => "BuildingAtlas",
                    MeshPhase::MeshingChunks => "MeshingChunks",
                    MeshPhase::Complete => "Complete",
                };
                Reflect::set(&obj, &"phase".into(), &JsValue::from_str(phase_str)).unwrap();
                Reflect::set(
                    &obj,
                    &"chunksDone".into(),
                    &JsValue::from(progress.chunks_done),
                )
                .unwrap();
                Reflect::set(
                    &obj,
                    &"chunksTotal".into(),
                    &JsValue::from(progress.chunks_total),
                )
                .unwrap();
                Reflect::set(
                    &obj,
                    &"verticesSoFar".into(),
                    &JsValue::from(progress.vertices_so_far as f64),
                )
                .unwrap();
                Reflect::set(
                    &obj,
                    &"trianglesSoFar".into(),
                    &JsValue::from(progress.triangles_so_far as f64),
                )
                .unwrap();
                let this = JsValue::NULL;
                let _ = callback.call1(&this, &obj.into());
            }));
    }

    /// Whether this iterator uses a shared global atlas.
    #[wasm_bindgen(getter, js_name = hasSharedAtlas)]
    pub fn has_shared_atlas(&self) -> bool {
        self.inner.has_shared_atlas()
    }

    /// Get the shared atlas (if any). Returns null if no shared atlas was set.
    #[wasm_bindgen(js_name = sharedAtlas)]
    pub fn shared_atlas(&self) -> Option<TextureAtlasWrapper> {
        self.inner.shared_atlas().map(|atlas| TextureAtlasWrapper {
            inner: atlas.clone(),
        })
    }
}
...
impl SchematicWrapper {
    ...
    #[wasm_bindgen(js_name = buildGlobalAtlas)]
    pub fn build_global_atlas(
        &self,
        pack: &ResourcePackWrapper,
        config: &MeshConfigWrapper,
    ) -> Result<TextureAtlasWrapper, JsValue> {
        build_global_atlas(&self.0, &pack.inner, &config.inner)
            .map(|atlas| TextureAtlasWrapper { inner: atlas })
            .map_err(|e| JsValue::from_str(&e.to_string()))
    }
    ...
    #[wasm_bindgen(js_name = chunkMeshIteratorWithAtlas)]
    pub fn chunk_mesh_iterator_with_atlas(
        &self,
        pack: &ResourcePackWrapper,
        config: &MeshConfigWrapper,
        chunk_size: i32,
        ...
        let iter = self.0.mesh_chunks_with_atlas(
            &pack.inner,
            &config.inner,
            chunk_size,
            atlas.inner.clone(),
