...
use crate::meshing::{ChunkMeshResult, MeshConfig, MeshOutput, RawMeshExport, ResourcePackSource};
...
impl PyMeshResult {
    ...
    /// Get NUCM binary cache data.
    pub fn nucm_data<'py>(&self, py: Python<'py>) -> Bound<'py, PyBytes> {
        let data = crate::meshing::cache::serialize_meshes(&[self.inner.clone()]);
        PyBytes::new(py, &data)
    }

    /// Save the mesh as a .nucm cache file.
    pub fn save_nucm(&self, path: &str) -> PyResult<()> {
        let data = crate::meshing::cache::serialize_meshes(&[self.inner.clone()]);
...
impl PyMultiMeshResult {
    ...
    /// Serialize all region meshes to a single NUCM binary cache.
    pub fn nucm_data<'py>(&self, py: Python<'py>) -> Bound<'py, PyBytes> {
        let meshes: Vec<MeshOutput> = self.inner.values().cloned().collect();
        let data = crate::meshing::cache::serialize_meshes(&meshes);
        PyBytes::new(py, &data)
    }

    /// Save all region meshes as a single .nucm cache file.
    pub fn save_nucm(&self, path: &str) -> PyResult<()> {
        let meshes: Vec<MeshOutput> = self.inner.values().cloned().collect();
        let data = crate::meshing::cache::serialize_meshes(&meshes);
        std::fs::write(path, &data)
...
impl PyChunkMeshResult {
    ...
    /// Serialize all chunk meshes to a single NUCM binary cache.
    pub fn nucm_data<'py>(&self, py: Python<'py>) -> Bound<'py, PyBytes> {
    ...
    /// Save all chunk meshes as a single .nucm cache file.
    pub fn save_nucm(&self, path: &str) -> PyResult<()> {
        let meshes: Vec<MeshOutput> = self.inner.meshes.values().cloned().collect();
        let data = crate::meshing::cache::serialize_meshes(&meshes);
        std::fs::write(path, &data)
...
/// Wrapper for a pre-built texture atlas.
///
/// Allows sharing a single atlas across multiple chunk meshes,
/// avoiding redundant atlas rebuilds.
#[pyclass(name = "TextureAtlas")]
pub struct PyTextureAtlas {
    pub(crate) inner: schematic_mesher::TextureAtlas,
}
...
impl PyTextureAtlas {
    ...
    pub fn rgba_data<'py>(&self, py: Python<'py>) -> Bound<'py, PyBytes> {
