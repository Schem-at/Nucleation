...
fn test_nibble_data_type_signal_strength() {
    ...
    let builder = SchematicBuilder::from_template(template)
        .expect("Failed to create builder from template");
    ...
    input_nbt.insert("Text1".to_string(), "{\"text\":\"@io.in=rc([0,-1,0],[0,-1,0])\"}".to_string());
    input_nbt.insert("Text2".to_string(), "{\"text\":\"#io.in:type=\\\"input\\\"\"}".to_string());
    input_nbt.insert("Text3".to_string(), "{\"text\":\"#io.in:data_type=\\\"nibble\\\"\"}".to_string());
    input_nbt.insert("Text4".to_string(), "{\"text\":\"\"}".to_string());
    
    annotated.set_block_with_nbt(
        0, 2, 2,
    ...
    output_nbt.insert("Text1".to_string(), "{\"text\":\"@io.out=rc([0,-1,0],[0,-1,0])\"}".to_string());
    output_nbt.insert("Text2".to_string(), "{\"text\":\"#io.out:type=\\\"output\\\"\"}".to_string());
    output_nbt.insert("Text3".to_string(), "{\"text\":\"#io.out:data_type=\\\"signal_strength\\\"\"}".to_string());
    ...
    let mut executor = create_executor_from_insign(&annotated)
        .expect("Failed to create executor from Insign");
    ...
    for test_value in test_values {
        ...
        let result = executor.execute(
            inputs,
            ExecutionMode::FixedTicks { ticks: 20 }
        ...
        // Check output value
        let output_value = result.outputs.get("out")
            .expect("Output 'out' not found");
        
        if let Value::U32(output_val) = output_value {
            assert_eq!(*output_val, test_value, 
                "Expected output to be {}, got {}", test_value, output_val);
        ...
        assert_eq!(output_power, test_value as u8,
            "Expected output wire power={}, got power={}", test_value, output_power);
